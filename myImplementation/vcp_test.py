...
for pol in range(len(arrPolyWKT)-1):
    ...
    for i in range(len(featurePointsSource)):
        ...
        regOfSupport = Ros(ros, featurePointsSource[i],transformed_polygo_src) #cria um objeto ROS
        featPoint = FeaturePoint(featurePointsSource[i],transformed_polygo_src, ros, rol, ror) #cria um objeto FP
    ...
    for fp in arrFPObjSource: #calcula as features de cada FP do source
        ...
        fp.getFeatures(fp.ros, transformed_polygo_src.length)
    ...
    correspondencesBetweenAllPoints = getIntermediateCorrespondences(transformed_polygo_src, arrPolyWKT[pol+1], pc1.getFPCorrespondences(3))
...
jaccard = getJaccardIndex(transformed_polygo_src, arrPolyWKT[pol+1], correspondences[i])
haussdorf = getHaussdorfDist(transformed_polygo_src, arrPolyWKT[pol+1], correspondences[i])
